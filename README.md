...
prompts/
├── finance/                   # 财经：选主题、生成初稿、改写定稿
└── basketball/                # 篮球：选主题、生成初稿、改写定稿
```

两个业务只拥有各自的采集实现。写作、去重、配图和发布均在公用层；公用采集接口按 `CONTENT_APP` 延迟加载一个采集器，因此财经进程不会加载篮球采集代码，篮球进程也不会加载财经采集代码。

财经和篮球都执行同一套三阶段写作链：`topic_selector.txt` 从真实素材选择话题，`article_generator.txt` 根据唯一来源生成初稿，`rewrite_article.txt` 对照来源改写成发布定稿。三个模板均在运行时加载，不是仅供测试或备用。

## 自动流程

1. 早晚从 World News API 读取国内财经科技新闻；中午读取与中国读者相关的海外财经科技新闻。
2. 调用各业务的选题 Prompt 筛选真实来源话题。
3. 调用各业务的文章生成 Prompt 生成初稿，再调用改写 Prompt 对照来源定稿。
